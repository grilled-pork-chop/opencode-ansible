...
.DEFAULT_GOAL := help
.PHONY: help setup lock fetch require-binary deploy check uninstall lint test clean
...
test: require-binary ## Run the Molecule install scenario (needs Docker)
	cd tests && $(UV) molecule test -s install

clean: ## Destroy Molecule containers and remove caches
